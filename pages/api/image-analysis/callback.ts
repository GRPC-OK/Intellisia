...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!versionId || !status) {
    return res.status(400).json({ message: 'Missing versionId or status in request body' });
  }

  if (!['success', 'fail'].includes(status)) {
    return res.status(400).json({ message: 'Invalid status value. Must be "success" or "fail".' });
  }
  ...
  if (status === 'success' && !s3Url) {
    ...
    console.warn(`Callback for versionId ${versionId} has status 'success' but no s3Url.`);
  }
  ...
  try {
    ...
    if (!updatedVersion) {
      return res.status(404).json({ message: `Version with ID ${versionId} not found during callback.` });
    }

    console.log(`Image analysis status updated for versionId ${versionId}: ${status}, S3 URL: ${s3Url || 'N/A'}`);
    return res.status(200).json({ message: 'Image analysis status updated successfully.' });
