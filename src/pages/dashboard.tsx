...
interface ProjectData {
  ...
  health: string;
  lastLog?: string | null;
  // TODO: 필요한 다른 필드 추가 (백엔드 API 응답에 포함된 필드)
}

// 프로젝트 상태별 아이콘 컴포넌트 (변경 없음)
const StatusIcon = ({ status }: { status: string }) => {
  if (status === 'approved') {
    return (
      <span title="승인" style={{ color: '#22c55e', fontSize: '1.5rem' }}>
        ✔️
      </span>
    );
  } else if (status === 'pending') {
    return (
      <span title="보류" style={{ color: '#eab308', fontSize: '1.5rem' }}>
        ▲
      </span>
    );
  } else {
    return (
      <span title="실패" style={{ color: '#ef4444', fontSize: '1.5rem' }}>
        ✖️
      </span>
    );
  }
...
// 예시 프로젝트 데이터 (하드코딩된 데이터 삭제)
// const projects = [...]; // <-- 이 부분 삭제
      ...
      <main className={styles.main}>
        ...
        <div className={styles.projectList}>
          ...
          {projects.map((project) => (
            ...
            >
              <Image
                src={project.icon || '/default-icon.png'} // icon이 null일 경우 기본 이미지 사용
                alt={project.name}
              ...
              <div className={styles.projectInfo}>
                ...
                <div style={{ fontSize: '0.85rem', color: '#94a3b8', marginTop: 2 }}>
                  최근 배포: {project.lastLog || '로그 없음'}{' '}
                  {/* lastLog가 null일 경우 대체 텍스트 */}
                </div>
              </div>
              {/* 배포 상태별 아이콘 (승인/보류/실패) */}
              <div
              ...
              >
                {/* project.size가 null일 경우 대체 텍스트 */}
                {project.size || '크기 정보 없음'}
