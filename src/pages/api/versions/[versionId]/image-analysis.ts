...
    }
    ...
    let sarifData;
    // 3. DB에 S3 URL이 있다면, 이제 S3에서 SARIF 파일 가져오기
    try { // S3 fetch 시도
...
        throw new Error(`Failed to fetch SARIF from S3. Status: ${sarifResponse.status} - ${errorText}`);
      }
...
      console.error(`[API /image-analysis] S3 fetch/parse failed. Returning mock data instead. Error: ${s3FetchError}`);
      sarifData = mockSarifData; // 실제 에러 대신 목업 데이터로 대체
    }


    // 4. 성공적인 응답 반환
    console.log(`[API /image-analysis] Successfully fetched SARIF for version ${versionId}.`);
